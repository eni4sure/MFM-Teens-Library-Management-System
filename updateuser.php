...
<?php
	...
	$SearchQueryParameter = $_GET['Update'];
				...
				$Query="SELECT * FROM users WHERE user_id = '$SearchQueryParameter'";
				$ExecuteQuery=mysqli_query($connection, $Query);
				while($DataRows=mysqli_fetch_array($ExecuteQuery)){
						$usernameToBeUpdated = $DataRows['user'];
						$fullnameToBeUpdated = $DataRows['fullname'];
						$classToBeUpdated = $DataRows['class'];
						$classTeacherToBeUpdated = $DataRows['classteacher'];
						$phonenumberToBeUpdated = $DataRows['phonenumber'];
						}
	...
	$classteacher=mysqli_real_escape_string($connection, $_POST['classteacher']);
	$phonenumber=mysqli_real_escape_string($connection, $_POST['phonenumber']);

		$show4 = "SELECT user FROM users WHERE user_id='$UpdateFromURL'";
		   $showcase4 = mysqli_query($connection, $show4);
		   $s4 = mysqli_fetch_assoc($showcase4);

		   
		  
		if($s4['user'] == $user ){
$update = "UPDATE `users` SET `user`='$user', `fullname`='$fullname', `class`='$class', `classteacher`='$classteacher', `phonenumber`='$phonenumber' WHERE user_id='$UpdateFromURL'";
$edit = mysqli_query($connection, $update);
if($edit){
	...
	$_SESSION["ErrorMessage"]="You cant change your username";
Redirect_to("users.php");
}
	...
	<section class="wrapper scrollable">
		...
		<div class="main-grid">
			...
			<div class="agile-grids">
				...
				<div class="form-body">
					<form class="form-horizontal" action="updateuser.php?Update=<?php echo $SearchQueryParameter; ?>" method="post" enctype="multipart/form-data">

						<div class="form-group"> 
							<label for="Username" class="col-sm-2 control-label">Username</label> 
							<div class="col-sm-9"> 
								<input type="text" name="username"   value="<?php echo $usernameToBeUpdated; ?>" class="form-control" id="Username" placeholder=" New Username"> 
							</div>
						</div>

						<div class="form-group"> 
							<label for="Book Title" class="col-sm-2 control-label">Fullname</label> 
							<div class="col-sm-9"> 
								<input type="text" name="fullname"  value="<?php echo $fullnameToBeUpdated; ?>" class="form-control" id="Book Title" placeholder="New Fullname"> 
							</div> 
						</div>

						<div class="form-group"> 
							<label for="Serial Number" class="col-sm-2 control-label">Class </label> 
							<div class="col-sm-9"> 
								<input type="text" name="class"  value="<?php echo $classToBeUpdated; ?>" class="form-control" id="Serial Number" placeholder="New Class"> 
							</div> 
						</div>

						<div class="form-group"> 
							<label for="Date Borrowed" class="col-sm-2 control-label">Class Teacher</label> 
							<div class="col-sm-9"> 
								<input type="text" name="classteacher"  value="<?php echo $classTeacherToBeUpdated; ?>" class="form-control"  placeholder="New Class Teacher"> 
							</div> 
						</div>

						<div class="form-group"> 
							<label for="Due Date" class="col-sm-2 control-label">Phonenumber</label> 
							<div class="col-sm-9"> 
								<input type="text" name="phonenumber"  value="<?php echo $phonenumberToBeUpdated; ?>" class="form-control" id="Due Date" placeholder="New Phonenumber"> 
							</div> 
						</div>
