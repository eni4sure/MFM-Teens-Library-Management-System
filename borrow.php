		...
		<div class="main-grid">
			...
			<?php
									...
									$Admin = $_COOKIE['username'];
							            ...
							            $dateborrowed = mysqli_real_escape_string($connection, $_POST['dateborrowed']);
										$datedue = mysqli_real_escape_string($connection, $_POST['datedue']);
										
										$Sqll = "SELECT COUNT(*) from $username WHERE datereturned ='Book not yet returned'";

										$rell = mysqli_query($connection, $Sqll);
										$ssll = mysqli_fetch_array($rell);
										$Totalel=array_shift($ssll);

										if($Totalel < 1){

							             $books  = "SELECT  statususer FROM users WHERE user = '$username'";
										    $collectbooks = mysqli_query($connection, $books);
										    $collectedbooks = mysqli_fetch_array($collectbooks);

										    if($collectedbooks['statususer'] == 'Enabled'){

										

										$status = "SELECT  status FROM books WHERE serialnumber = '$serialnumber' AND title = '$title'";
										...
										if ($statuskeyword['status'] == 'Available'){
											...
											if(($username != null ) && ($title != null)  && ($dateborrowed != null) && ($datedue != null) ){
												...
												$sql = "INSERT INTO $username( book, serialnumber, dateborrowed, datedue, datereturned, chek, borrowadmin, returnadmin) 
 								                VALUES ('$title','$serialnumber', '$dateborrowed', '$datedue',  'Book not yet returned', '1', '$Admin','Not yet returned')";

 								                $sqli = "INSERT INTO borrowdetails( username, book, serialnumber, dateborrowed, datedue, datereturned, borrowadmin, returnadmin)
 								                VALUES ('$username', '$title', '$serialnumber', '$dateborrowed', '$datedue', 'Book not yet returned', '$Admin', 'Not yet returned')";

 								          $conn1 = mysqli_query($connection, $sqli);
 								          $conn2 = mysqli_query($connection, $sql);
 								       if($conn2) {
									         $update = "UPDATE books SET status ='Unavailable' WHERE serialnumber = '$serialnumber' ";
										$statusupdate = mysqli_query($connection, $update);
										...
										<a href=\"#\" class=\"close\" data-dismiss=\"alert\" aria-label=\"close\">&times;</a>
										  <strong>Success!</strong><br/>Operation was successful</div>";
											...
											echo "<div class=\"alert alert-danger alert-dismissable\">
											<a href=\"#\" class=\"close\" data-dismiss=\"alert\" aria-label=\"close\">&times;</a>
											<strong> Danger This book is not available</strong>"."<br/></div>";
										}
										...
										echo "<div class=\"alert alert-danger alert-dismissable\">
										<a href=\"#\" class=\"close\" data-dismiss=\"alert\" aria-label=\"close\">&times;</a>
										<strong> Danger This user has been deleted</strong>"."<br/></div>";
									}
								}else{
									echo "<div class=\"alert alert-danger alert-dismissable\">
									<a href=\"#\" class=\"close\" data-dismiss=\"alert\" aria-label=\"close\">&times;</a>
									<strong> Danger You cant borrow more than 1 book</strong>"."<br/></div>";
								}
									} 
									?>
				...
				<div class="form-body">
					<form class="form-horizontal" action="borrow.php" method="post">
						...
						<div class="form-group"> 
							...
							<div class="col-sm-9"> 
								<input type="date" name="dateborrowed" class="form-control" id="Date Borrowed" placeholder="Date Borrowed"> 
							</div> 
						</div>
						...
						<div class="form-group"> 
							...
							<div class="col-sm-9"> 
								<input type="date" name="datedue" class="form-control" id="Due Date" placeholder="Due Date"> 
							</div> 
						</div>

						<div class="col-sm-offset-2"> 
							<button name="submit" type="submit" class="btn btn-default w3ls-button">Submit</button> 
						</div> 
					</form> 
				</div>
			</div>
